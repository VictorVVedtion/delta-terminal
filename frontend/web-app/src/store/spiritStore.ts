...
import { SpiritEvent as RealtimeSpiritEvent } from '../lib/spiritRealtime';
...
export const useSpiritStore = create<SpiritStore>((set, get) => ({
  ...
  setEvent: (event: SpiritEvent) => set((state) => {
    // 如果事件包含状态，则更新当前状态
  ...
  checkHeartbeat: () => {
    ...
    if (Date.now() - lastHeartbeat > 15000 && currentState !== 'dormant' && currentState !== 'error') {
