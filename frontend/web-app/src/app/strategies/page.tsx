...
import { Activity, Archive, Bot, Grid, Trash2, TrendingUp, Zap } from 'lucide-react'
...
import { ArchivedStrategyView, RecycleBinPage } from '@/components/strategy'
...
function StrategiesContent() {
  ...
  const { getArchivedStrategies, getDeletedStrategies } = useStrategyLifecycleStore()

  // Hydrate store on mount
  const hydrated = useStrategyLifecycleHydration()

  // Get strategies (only after hydration to avoid mismatch)
          ...
          <Tabs defaultValue={tab} className="w-full">
            <TabsList className="grid w-full max-w-md grid-cols-3">
              ...
              <TabsTrigger value="archived" className="gap-1.5">
                ...
                归档 ({archivedStrategies.length})
              </TabsTrigger>
              <TabsTrigger value="deleted" className="gap-1.5">
                <Trash2 className="h-3.5 w-3.5" />
                回收站 ({deletedStrategies.length})
              </TabsTrigger>
            </TabsList>
